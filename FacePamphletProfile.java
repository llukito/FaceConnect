...
public class FacePamphletProfile implements FacePamphletConstants {
	...
	public FacePamphletProfile(String name) {
		this.name = name.trim();
		status = "";
		image = null; // it's extra but still :)
	 ...
	 * the names of all of the friends in this profile. For example, in a
	 * profile with name "Alice" whose status is "coding" and who has friends
	 * Don, Chelsea, and Bob, this method would return the string:
	 * "Alice (coding): Don, Chelsea, Bob"
	 */
	public String toString() {
		String result = name + " "; // add name
		result += "(" + status + ") : "; // add status
		for (int i = 0; i < friendsList.size(); i++) { // add friends
