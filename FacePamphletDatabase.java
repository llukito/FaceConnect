...
public class FacePamphletDatabase implements FacePamphletConstants {
	...
	/*
	 * Instance variable of map,which stores profiles as values with keys of
	 * names
	 * I made it public for extension ( because I check whether it's
	 * empty or not while opening chat(Extension of messenger))
	 */
	public Map<String, FacePamphletProfile> profilesMap;
	 ...
	 * friends of any other profile. If there is no profile in the database with
	 * the given name, then the database is unchanged after calling this method.
	 */
	public void deleteProfile(String name) {
		if (profilesMap.containsKey(name)) {
			profilesMap.remove(name); // from map
			for (FacePamphletProfile profile : profilesMap.values()) {
				profile.removeFriend(name); // from list of friends
			}
		}
	}
